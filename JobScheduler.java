...
//to sort process based on arrival time
	...
	//driver code
	/*
	input specifications number of process,and process details arrivaltime and burst time
	output specifications waiting time,turn around time,completetion time,avg waiting time,maximum waiting time in queue
	*/
    ...
    {   
        ...
        int n;
		n=j1.input();
        System.out.println("now enter arival time and burst time for all processes");
        int[][] input = new int[4][2]; 
        for (int i = 0; i < n; i++) 
        {
            System.out.println("enter arival time and burst time for  processe  "+(i+1));
            for (int j = 0; j < 2; j++) 
                 input[i][j]=j1.input();
        } 
		j1.sort(input,n);		
