...
/**
 * Undirected Weighted Graph Implementation by  Matrix Representation
 * @author Bhavesh
 *
 */
public class GraphByMatrix implements UndirectedGraphInterface {
	int graphMatrix[][];
	int graphSize=4;
	 ...
	 */
	public HashSet connection(int vertex,HashSet allVertexList)
	{
		for(int i=1;i<=graphSize;i++)
		{
			if(graphMatrix[vertex][i]>0)
			{
				if(!allVertexList.contains(i))
				{
					...
					allVertexList=this.connection(i, allVertexList);
				}
	...
	public boolean isConnected()
	{
		...
		allVertexList=this.connection(1,allVertexList);
		 if(allVertexList.size()==this.graphSize)
		 {
			 return true;
		 }
		 else
	...
	public static void main(String []args)
	{
		...
		else
		{
		 ...
		 ArrayList<Integer> reachables= graph.reachable(1);
		 System.out.println("Connected Nodes are "+reachables);
